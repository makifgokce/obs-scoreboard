...
{
    public class CounterUpdateHandler : IMessageHandler
    {
        private readonly ScoreData _scoreData;
        private readonly IWebSocketServer _server;

        public CounterUpdateHandler(ScoreData scoreData, IWebSocketServer server)
        {
        ...
        public async Task HandleMessageAsync(WebSocket socket, string message)
        {
            try
            {
                var update = JsonSerializer.Deserialize<CounterUpdate>(message);
                if (update != null)
                {
                    _scoreData.Counter = update.Value;
                    _scoreData.CounterName = update.Name;
                    await BroadcastUpdate();
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error handling counter update: {ex.Message}");
            }
        }
